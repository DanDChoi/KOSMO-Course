...
-- 최대 급여를 받는 사원의 이름을 출력 
   ...
   (4) 단일행 함수
	  ...
	  <14> least('문자열1', '문자열2', '문자열3')
	   ...
	   <7> tan(숫자[rad])
	      SQL> select tan(45* 3.141592/180) from dual;
	   ...
	   <3> last_day(날짜컬럼 or 날짜데이터) 
	      SQL> select last_day('21/12/21') from dual;
	   ...
	   <9> 'YY-MM-DD'
	      SQL> select SYSDATE, to_char(SYSDATE, 'YY-MM-DD') from DUAL;
	   <10> 'HH' or 'HH12'
	   ...
	   <14> 'AM' or 'PM'
	      ...
	      --현재날짜를 '2021-12-21 오전 11:43:10 화요일' 식으로 출력
	      ...
	      Tip2)  
		select SYSDATE, to_char(SYSDATE, 'DDSP') from DUAL; -- one two three .. 
   ...
   (5) 복수행 함수 ( group function ***** )
       1) count(컬럼명)
           -- 사원의 수를 출력 
	   SQL> select count(EMPNO) from EMP; --PK컬럼 사용하면 안전
	   ...
	   SQL> select ENAME, SAL from EMP where SAL=min(SAL);--(X) 그룹함수는 where절X
       ...
       SQL1> select DEPTNO, avg(SAL) from EMP group by DEPTNO;
       SQL2> select DEPTNO, round(avg(SAL)) from EMP group by DEPTNO;
   ...
   #3 Join & SubQuery 
    (1) 조인( `Join` )
	  ...
	  <3> Self 조인
	      ...
	      1> 형태1
	      SQL> select e.ENAME, m.ENAME from EMP e, EMP m where e.MGR=m.EMPNO; --뼈대
	      ...
	      2> 형태2
	      SQL> select e.ENAME, m.ENAME from EMP e join EMP m on e.MGR=m.EMPNO; --뼈대
	...
	4) 종류
	       ...
	       -- 부서번호가 10번인 사원급여와 급여가 같은 사원의 이름과 커미션을 출력(X:복수행)
	       ...
	       Err> select ENAME, COMM from EMP where SAL=(select SAL from EMP where DEPTNO=10);
               ...
               -- 사번이 7369인 사원의 급여와 커미션이 같은.. 사원의 이름과 커미션을 출력(X:복수컬럼) 
	   ...
	   <2> 복수행 Sub-Query
		 ...
		 SQL> select DNAME from DEPT where exists(select * from EMP where DEPTNO=10);
	   ...
	   <3> 복수컬럼 Sub-Query
	       ...
	       Main> select ENAME, DEPTNO from EMP where (SAL, COMM) in ? 
	 ...
	 Sub> select avg(SAL)from emp where DEPTNO=10;
	 SQL> select deptno "부서번호", round(avg(sal)) "평균급여" from emp 
	      where sal<(select avg(sal) from emp where deptno=10)
	      group by deptno
	      having deptno != 10
	      order by deptno desc;
...
2. DML ( Data Manipulation Language ) 
   ...
   (2) 종류 
	  ...
	  SQL> insert into EMP2 values(8000, '홍길동', '개발자', 7839, SYSDATE, 8000, null, 50);

          Err> insert into EMP2(EMPNO, ENAME, SAL, HIREDATE) values(8000, '이순신', 7000, SYSDATE); --에러(PK)
	  SQL> insert into EMP2(EMPNO, ENAME, SAL, HIREDATE) values(9000, '이순신', 7000, SYSDATE);

	  Err> insert into EMP2(ENAME, SAL, HIREDATE) values('강감찬', 6000, SYSDATE); --에러(PK)
	  SQL> insert into EMP2(EMPNO, SAL, HIREDATE) values(9999, 6000, SYSDATE);

          확인> select * from EMP2 where EMPNO>=8000;

       2) update 
          SQL> update EMP2 set ENAME='홍길순', SAL=8500 where EMPNO=8000;
	  SQL> update EMP2 set EMPNO=8001 where EMPNO=8000; --PK컬럼도 가능 
	  SQL> update EMP2 set DEPTNO=10 where EMPNO=8001; --FK컬럼도 가능

       3) delete  
          SQL> delete from EMP2 where EMPNO=9999;
	  SQL> delete from DEPT2 where DEPTNO=30;
	   -- on delete cascade 옵션으로 FK제약조건이 부여되었다면 자식데이터존재유무와 상관없이 삭제 됨
...
---------------------------- SQL 끝 --------------------------------
	...
	- 테이블들(제약조건 포함)을 만들고(DDL), 
	- 데이터 조작(insert/update/delete)하고(DML), 
	- 고객이 원하는 결과(DQL)의 샘플을 만드시오
...
3. 일련번호 ( Sequence )
   ...
   (3) 생성 예 
       SQL> create sequence DEPT2_SEQ increment by 1 start with 1 nocache; --생성 
       SQL> desc seq -- 딕셔너리 확인
       SQL> select SEQUENCE_NAME, INCREMENT_BY, MIN_VALUE, MAX_VALUE from seq;--확인
   ...
   (5) 삭제 
       SQL> drop sequence DEPT2_SEQ; --삭제
       SQL> select  * from seq; --확인 
      
   (6) 일반적 사용예 
       SQL> create table ST(seq number primary key, name varchar2(10));
       SQL> create sequence ST_SEQ increment by 1 start with 1 nocache;
       SQL> insert into ST values(ST_SEQ.nextval, '홍길동');
       SQL> insert into ST values(ST_SEQ.nextval, '이순신');
       SQL> insert into ST values(ST_SEQ.nextval, '강감찬');
       SQL> select * from ST;
...
4. 제약조건 ( Constraint ***** )     
   ...
   (2) 종류 
       ...
       3) unique key ( 유일키 )
          PK가 아니더라도 컬럼의 모든 값이 유일해야 하는 경우에 사용되는 키 
...
2. 복구( imp.exe )   
   ...
   (2) 특정 사용자 ( User Level Import ) ***** 
       ...
       #시도1
       C:\>imp userid=system/java1234 file='C:\SOO\Oracle\4_BACKUP\dump1.dmp' fromuser=scott touser=scott2

       1) 발생된 에러 메세지 
           KO16MSWIN949 문자집합과 AL16UTF16 NCHAR 문자 집합에 임포트가 완성 되었습니다
           AL32UTF8 문자 집합을 사용해서 서버를 임포트합니다 (문자집합 변환이 가능합니다)
           IMP-00030: 쓰기 모드로 import_sys 파일을 만드는데 실패했습니다
           IMP-00000: 임포트가 실패로 끝났습니다
    
       2) 해결 시도 
           <1> 현재 오라클의 문자셋을 조회  ( 관리자 계정에서 )
	      실행창> sqlplus / as sysdba
	      SQL> select * from nls_database_parameters where parameter='NLS_CHARACTERSET';
	           -- AL32UTF8

           <2> AL32UTF8을 KO16MSWIN949로 변경 ( 관리자 계정에서 )
	...
	2) 해결 시도 
	   '관리자모드로 명령프롬프트'를 열어서, 복구 시도!  
        ...
        3) 원래 문자셋으로 변경 ( KO16MSWIN949 -> AL32UTF8 ) 
	      실행창> sqlplus / as sysdba
	      SQL> shutdown immediate;
	      SQL> startup mount;
	      SQL> alter system enable restricted session;
   ...
   (3) 특정 테이블 ( Table Level Import )
       SQL>alter session set "_oracle_script"=true; 
       SQL>create user scott3 identified by tiger3; 
       SQL>grant connect, resource, unlimited tablespace to scott3;
      ...
      <핵심> ( AL32UTF8 -> KO16MSWIN949 ) -> 백업/복구('관리자모드CMD') -> ( KO16MSWIN949 -> AL32UTF8 )	 
...
1. Procedure
   (1) 설명 
       일련의 '작업처리순서'를 정의해 놓은 것으로 미리 DBMS에 컴파일되어져 있어서,  
...
1. DB 모델링이란 ?
   ...
   SW로 구현해내기 위한 '데이터'구조의 뼈대를 작성하는 업무 
	...
	2) 예 - 고객 table 보기 변경 
	   ...
	   <2> 컬럼(COLUMN)을 기준 
        ...
        4) 관계 (능동적으로 표현)
          ...
          질문> 이름이 '홍길동'인 자동차의 차넘버를 SELECT 하라
...
3. 단계( step )
  ...
  (2) DB 모델링 순서 
	...
	ex2) 부서/사원 ( 1: N )
     ...
     3) 논리적 모델링
	  ...
	  스키마를 생성하고 '정규화'까지를 포함하는 단계 
	  ( 스키마는 개체들의 관계 제약조건등의 명세를 포함의 데이터 구조 )
	 ...
	 <2> drop.sql 
	 <3> insert.sql 
	 <4> select.sql ( '요구 사항'을 기준으로 작성 ) 


   (3) ERD의 관계 표기 
     1) 표기 
       <1> 실선 : '식별'관계 (Identifying Type)
          (부모 테이블의 PK가 자식테이블의 FK/PK가 되는 경우)
       <2> 점선 : '비식별'관계 (Non-Identifying Type)
  ...
  (6) 모델링 툴(exerd) 사용 
     ...
     2) 논리 모델링 
         <1> 테이블 생성 
             플젝 생성 -> exerd 파일 생성  -> 테이블 생성 -> Cnt + Enter 
