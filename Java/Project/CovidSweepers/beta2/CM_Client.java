...
public class CM_Client extends JFrame implements ActionListener
{
   ...
   public CM_Client() {
     ...
     label_Flag1_syringe = new Label("[ 라운드점수 ]");
     ...
     label_Flag2_syringe = new Label("[ 라운드점수 ]");
     ...
     label_Flag3_syringe = new Label("[ 라운드점수 ]");
     ...
     label_Flag4_syringe = new Label("[ 라운드점수 ]");
...
/*----------211214-----------------*/
  ...
  class Sender extends Thread implements KeyListener, ActionListener {
   ...
   class Score2 // extends Thread
   {
      Socket s;
      DataOutputStream dos;
      String Nick = CM_Login.nickName;// 클라이언트에 데이터를 뿌리기 위해 사용
   ...
   public void updateClientList() { // 클라이언트 목록 추가
      ...
      if (Integer.parseInt(playerIdx) == 0) {
         ...
         ii = new ImageIcon("image\\YC.jpg");
         ii.getImage().flush();
         laClient4.setIcon(ii);
         laClient4sub.setText("[" + playerName + "]");

         sy = new ImageIcon("image\\YS.png");
         sy.getImage().flush();
