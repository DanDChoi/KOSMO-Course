...
alter table CT_EMP add constraint CT_EM_PK primary key(NO);
alter table CT_EMP add constraint CT_EM_CK check(ADDR in ('서울', '부산'));
alter table CT_EMP add constraint CT_EM_UQ unique(JUMIN);
alter table CT_EMP add constraint CT_EM_FK foreign key(DEPTNO) references CT_DEPT(DEPTNO);
select constraint_name, constraint_type from user_constraints 
